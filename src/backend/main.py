...
async def export_mujoco_urdf(
    ...
    try:
        ...
        with open("DEBUG_EXPORT_START.txt", "w") as df:
            ...
            if recordings:
                df.write(f"Recordings Length: {len(recordings)}\\n")
                df.write(f"Preview: {recordings[:100]}\\n")
            else:
                df.write("Recordings: NONE\\n")
...
"""
                ...
                validation_py = generate_validation_script(
                    urdf_filename, recording_data
                )
                with open(
                    os.path.join(package_dir, "validate_motor_params.py"), "w"
                ) as f:
