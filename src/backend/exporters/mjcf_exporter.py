...
def generate_mjcf_xml(robot: RobotData, robot_name: str, mesh_files_map: Dict[str, str], unique_link_names: Dict[str, str], use_mesh_collision: bool = False, direct_hand: bool = False) -> Tuple[str, List[Dict]]:
    ...
    xml.append('  <option timestep="0.002" iterations="50" solver="Newton" tolerance="1e-10" gravity="0 0 -9.81"/>')
        ...
        if link.visual and link.visual.type != 'none':
            ...
            if v.type == 'mesh' and link_id in mesh_files_map:
                 ...
                 if use_cylinder:
                     ...
                     coll_geom = 'type="cylinder" size="0.012 0.02" pos="0.02 0 0" euler="0 1.5708 0" group="0" rgba="0 0 1 0.4" condim="3" solref="0.01 1" solimp="0.95 0.99 0.001" margin="0.002" contype="1" conaffinity="1"'
                     xml.append(f'{indent}  <geom {coll_geom} />')
                 else:
                     # Detailed Mesh 
                     coll_geom = f'type="mesh" mesh="{asset_name}" group="0" rgba="1 0 0 0" condim="3" solref="0.01 1" solimp="0.95 0.99 0.001" margin="0.002" contype="1" conaffinity="1"'
