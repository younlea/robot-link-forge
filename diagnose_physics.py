...
def diagnose_mjcf(model_path):
    ...
    if extreme_inertia:
        ...
        for name, inertia in extreme_inertia[:5]:
            print(
                f"     - {name}: [{inertia[0]:.2e}, {inertia[1]:.2e}, {inertia[2]:.2e}]"
            )
    ...
    if total_mass > 50:
        issues.append(
            f"Robot very heavy ({total_mass:.1f} kg) - need high motor torque"
        )
    elif total_mass < 0.1:
    ...
    if strong_actuators:
        issues.append(
            f"{len(strong_actuators)} extremely strong actuators - may cause instability"
        )

    if high_kp:
        issues.append(
            f"{len(high_kp)} actuators with very high kp - causes oscillations"
        )

    if low_damping:
        issues.append(
            f"{len(low_damping)} actuators with low damping - causes oscillations"
        )
